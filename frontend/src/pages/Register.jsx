...
const fieldDefs = [
  ...
  { name: "voterId", label: "Voter ID", type: "text", icon: CreditCard, placeholder: "12-digit number", span: true },
  { name: "email", label: "Email", type: "email", icon: Mail, placeholder: "you@example.com" },
  { name: "phone", label: "Phone Number", type: "tel", icon: Phone, placeholder: "10-digit mobile number" },
...
export default function Register() {
  const { values, errors, touched, handleChange, handleBlur, validateForm, reset } = useFormValidation(
    { fullName: "", voterId: "", email: "", phone: "", constituency: "", dateOfBirth: "" },
    schema
  );
  const [descriptor, setDescriptor] = useState(null);
  const [message, setMessage] = useState("");
  const [submitting, setSubmitting] = useState(false);
  const [formVersion, setFormVersion] = useState(0); // bump to force FaceCapture to remount + reset
  ...
  return (
    <AuthSplitLayout>
      <div className="mb-4 flex items-center gap-2">
        ...
        <div>
          <h1 className="text-lg font-bold text-ink-900 dark:text-white">Voter Registration</h1>
          <p className="text-sm text-slate-500 dark:text-slate-400">Reviewed by an admin before you can vote</p>
        </div>
      </div>
      ...
      <div className="card card-hover animate-fade-in">
        <form key={formVersion} onSubmit={handleSubmit} className="flex flex-col gap-4" noValidate>
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-bold text-ink-900 dark:text-white">Your details</h3>
          </div>
          ...
          <div className="border-t border-slate-200 pt-4 dark:border-slate-700">
            <div className="mb-3 flex items-center gap-2">
              ...
              <h3 className="text-sm font-bold text-ink-900 dark:text-white">Capture your face</h3>
            </div>
            <FaceCapture
